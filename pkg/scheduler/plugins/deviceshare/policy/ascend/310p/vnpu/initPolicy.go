...
func addNPUResourceVNPUCard(device *vnpu.NPUDevices, pod *v1.Pod, chipTotalRes vnpu.VResource,
	...
	if podVResource == nil {
		klog.V(vnpu.LogErrorLev).Infof("addNPUResource resolving pod<%s> resource failed", pod.Name)
		return
	}
...
func getPodUsedRes(device *vnpu.NPUDevices, pod *v1.Pod, taskTemplate map[string]map[string]vnpu.VResource) *vnpu.VResource {
	realStr, ok := pod.Annotations[vnpu.AscendNPUCore]
	if !ok {
		klog.V(vnpu.LogErrorLev).Infof("getPodUsedRes get pod<%s> %s value failed", pod.Name,
			vnpu.AscendNPUCore)
		return nil
	}
	ascendRealSplit := strings.Split(realStr, "-")
	if len(ascendRealSplit) != vnpu.NPUIndex2 {
		klog.V(vnpu.LogErrorLev).Infof("getPodUsedRes get pod<%s> %s format error", pod.Name, realStr)
		return nil
	}
...
func NewVChip(device *vnpu.NPUDevices, id int, totalRes vnpu.VResource) *vnpu.VChip {
	if device == nil {
		klog.V(vnpu.LogDebugLev).Infof("NewVChip failed: %s", vnpu.ArgumentError)
		return nil
	}
	chipName := device.ChipKind + "-" + strconv.Itoa(id)
	vChip := vnpu.VChip{
		PodMap:   make(map[string]*v1.Pod, vnpu.MapInitNum),
		Name:     chipName,
...
func getVNPUCardIDFromAscendCore(coreNameStr string) (int, error) {
	coreNameSplit := strings.Split(coreNameStr, "-")
	if len(coreNameSplit) != vnpu.NPUIndex2 {
...
func isPodWholeCardFromAscendCore(coreCardName string) bool {
	...
	for _, cardName := range temp {
		...
		if len(singleCardTemp) == vnpu.NPUIndex1 {
			return true
		}
	}
	return false
...
func getCardIDsFromNodeAndDeviceInfo(device *vnpu.NPUDevices, cardHealthTypeSuffix string) ([]int, error) {
	...
	for _, chip := range Chips {
		...
		if aErr != nil {
			klog.V(vnpu.LogDebugLev).Infof("%s %s covert to int %s", chip, strID, vnpu.SafePrint(aErr))
			continue
		}
...
func createNodeNewVChips(device *vnpu.NPUDevices, chipTotalRes vnpu.VResource) error {
	...
	if getErr != nil {
		return fmt.Errorf("getFreeCardIDsFromDeviceInfo %s", vnpu.SafePrint(getErr))
	}
	klog.V(vnpu.LogDebugLev).Infof("createNodeNewVChips healthy chips: %#v", healthyCardIDs)
